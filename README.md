...
`lottery-dcb` 是基于 Spring Boot 4.1 和 Vue 3 的双色球组合约束、7+2 复式、历史统计与旋转矩阵覆盖工具
...
- 网页默认使用 7+2 复式并生成 2 组，也可按需调整为 1–10 组
- 生成结果支持一键复制，红球、加号和蓝球使用双空格分隔，各组之间保留一个空行
- 桌面端复式结果固定为 2 列；前 4 组以 2×2 展示，第 5 组开始向下排列并在结果区内滚动
- 组合指标解释、号码覆盖率、二码覆盖率、红蓝球频次统计
- 红球 1–33、蓝球 1–16 分表展示的遗漏走势图，每种球的全部号码连续显示，支持最近 20/30/50/100 期
- 开奖数据按期号、日期、红球、蓝球展示，红蓝球样式与号码走势保持一致
- 页面固定为单个浏览器视口，通过顶部导航切换首页、生成结果、历史统计、号码走势和开奖数据
- 不使用任何翻页按钮或分页状态，当前加载的走势图、历史开奖和生成结果一次完整渲染
...
- **首页**：配置复式或单式生成参数、历史观察期数、蓝球选择方式和各项硬约束。默认打开 `7+2 复式`，复式组数为 `2`
...
完整 Maven 构建会依次执行前端依赖安装、前端测试、Vite 构建、Java 编译和后端测试；任一步失败都会终止打包，避免生成包含旧页面或不完整资源的 jar
